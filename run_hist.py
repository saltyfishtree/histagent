...
def get_examples_to_answer(answers_file, eval_ds, args=None) -> List[dict]:
    ...
    if args:
        ...
        if args.question_ids:
            ...
            # Filter questions by comparing integer task_ids
            for example in examples:
                task_id = example.get("task_id")
                if isinstance(task_id, int) and task_id in target_ids:
                    filtered_examples.append(example)
                elif isinstance(task_id, str) and task_id.isdigit() and int(task_id) in target_ids:
                    filtered_examples.append(example)
